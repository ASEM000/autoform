...
@ft.partial(af.core.impl_rules.set, shout_p)
...
@ft.partial(af.core.pull_fwd_rules.set, shout_p)
def pull_fwd_shout(text: str) -> tuple[str, str]:
    return text.upper(), text


@ft.partial(af.core.pull_bwd_rules.set, shout_p)
def pull_bwd_shout(in_tree) -> str:
    residual, cotangent = in_tree
    return cotangent


@ft.partial(af.core.batch_rules.set, shout_p)
def batch_shout(in_tree) -> tuple[list[str], bool]:
    batch_size, in_batched, text = in_tree
    if in_batched:
...
@ft.partial(af.core.impl_rules.set, textgrad_style_lm_call_p)
...
@ft.partial(af.core.pull_bwd_rules.set, textgrad_style_lm_call_p)
def pull_bwd_textgrad_style_lm_call(
    ...
    *,
    ...
    if hasattr(out_cotangent, "model_dump"):
        cotangent_str = str(out_cotangent.model_dump())
    else:
        cotangent_str = str(out_cotangent)
...
@ft.partial(af.core.push_rules.set, textgrad_style_lm_call_p)
def push_textgrad_style_lm_call(in_tree, *, roles: tuple, model: str, struct: type[af.Struct]):
...
@ft.partial(af.core.batch_rules.set, textgrad_style_lm_call_p)
def batch_textgrad_style_lm_call(in_tree, *, roles: tuple, model: str, struct: type[af.Struct]):
