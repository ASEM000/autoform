...
class TestSwitchIRStructure:
    def test_creates_switch_eqn(self):
        ...
        assert ir.eqns[0].prim.name == "switch"

    def test_has_branches_param(self):
        branches = {
            "x": af.trace(lambda x: x)("X"),
            "y": af.trace(lambda x: x)("X"),
        }
        ...
        assert "branches" in ir.eqns[0].params
        assert len(ir.eqns[0].params["branches"]) == 2
