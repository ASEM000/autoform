...
class TestSwitchPushforward:
    def test_pushforward_selects_correct_branch(self):
        ...
        primals = ("zero", "hello")
        tangents = ("", "world")
        p_out, t_out = af.run_ir(pf_ir, (primals, tangents))
...
class TestSwitchPullback:
    def test_pullback_key_zero(self):
        ...
        primals = ("zero", "hello")
        cotangent = "grad"
        _, (c_key, c_x) = af.run_ir(pb_ir, (primals, cotangent))
...
class TestSwitchNestedTransforms:
    def test_pushforward_of_batch(self):
        ...
        primals = ("a", ["a", "b"])
        tangents = ("", ["ta", "tb"])
        p_out, t_out = af.run_ir(pf_batched_ir, (primals, tangents))
    ...
    def test_pullback_of_batch(self):
        ...
        p_out, c_in = af.run_ir(pb_batched_ir, (primals, cotangents))
        assert p_out == ["A:a", "A:b"]
        c_key, c_x = c_in
