...
from autoform.axis import BatchAVal, BatchBox, BatchInterpreter
from autoform.order import fanout_p
...
class TestBatchBasic:
    def test_single_arg(self):
        def shout(text):
            return af.string.format("{text}!", text=text)
    ...
    def test_nested_format(self):
        def template(name, value):
            ...
            return af.string.format("{name}: {inner}", name=name, inner=inner)
...
class TestBatchIRStructure:
    def test_batch_aval_tangent_space(self):
...
class TestBatchRuleOutBatched:
    def test_format_out_batched_is_scalar(self):
        ir = af.trace(lambda x: af.string.format("{x}", x=x))("a")
        (eqn,) = ir.eqns
        batch_size = 2
        in_batched = (True,)
        in_values = (["a", "b"],)
        out_vals, out_batched = af.core.batch_rules.get(eqn.prim)((
            batch_size,
            in_batched,
            in_values,
        ))
...
class TestBatchBroadcasting:
    ...
    def test_format_mixed_batched(self):
        ir = af.trace(
            lambda x, greeting: af.string.format("{greeting}, {x}!", x=x, greeting=greeting)
        )("a", "Hello")
        (eqn,) = ir.eqns
        batch_size = 2
        in_batched = (False, False, True, False)
        in_values = ("Hello", ", ", ["x0", "x1"], "!")
        out_vals, out_batched = af.core.batch_rules.get(eqn.prim)((
            batch_size,
            in_batched,
            in_values,
...
class TestBatchRuleAllUnbatched:
    def test_format_all_unbatched(self):
        ir = af.trace(lambda x, y: af.string.format("{x} {y}", x=x, y=y))("hello", "world")
        (eqn,) = ir.eqns
        batch_size = 3
        in_batched = (False, False, False)
...
class TestFanoutWithDCE:
    ...
    def test_fanout_dce_propagates_to_branches(self):
        def program(x):
            live = af.string.format("[{x}]", x=x)
            dead = af.string.format("<{x}>", x=x)
            return live
    ...
    def test_fanout_unused_branch_structured_output_is_callable(self):
        ...
        ir_live = af.trace(lambda x: af.string.concat(x, "!"))("x")
...
class TestSchedRecursive:
        ...
        branches = {
            "a": af.trace(
                lambda x: af.string.concat(
                    af.string.format("[{x}]", x=x), af.string.format("<{x}>", x=x)
                )
            )("x"),
            "b": af.trace(lambda x: af.string.format("({x})", x=x))("x"),
        }
    ...
    def test_sched_nested_switch(self):
        inner_branches = {
            "x": af.trace(
                lambda a: af.string.concat(
                    af.string.format("[{a}]", a=a), af.string.format("<{a}>", a=a)
                )
            )("a"),
            "y": af.trace(lambda a: af.string.format("({a})", a=a))("a"),
        }
        ...
        outer_branches = {
            "A": inner_ir,
            "B": af.trace(lambda key, inp: af.string.format("{key} {inp}", key=key, inp=inp))(
                "k", "i"
            ),
        }
    ...
    def test_sched_with_cond_propagates_to_nested(self):
        branches = {
            "a": af.trace(lambda x: (af.string.format("[{x}]", x=x), af.string.concat(x, "!")))(
                "x"
            ),
        }
    ...
    def test_sched_preserves_non_ir_params(self):
        branches = {
            "a": af.trace(lambda x: af.string.format("[{x}]", x=x))("x"),
        }
...
class TestDependsWithPushforward:
    ...
    def test_pushforward_chained(self):
        def program(x):
            ...
            c = af.depends(af.string.format("C: {x}", x=x), b)
            return c
...
class TestDependsWithSched:
    ...
    def test_sched_data_dependency(self):
        def program(x):
            ...
            b = af.string.format("B: {a_barrier}", a_barrier=a_barrier)
            return b
...
class TestFanoutBatchAllUnbatched:
    def test_fanout_batch_all_unbatched(self):
        ir1 = af.trace(lambda x: af.string.format("[{x}]", x=x))("a")
        ir2 = af.trace(lambda x: af.string.format("<{x}>", x=x))("a")
        irs = [ir1, ir2]
