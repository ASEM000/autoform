...
- calls to `autoform` primitives such as {py:func}`concat <autoform.string.concat>`, {py:func}`af.lm.complete <autoform.lm.complete>`, {py:func}`switch <autoform.switch>`, and {py:func}`while_loop <autoform.while_loop>` become IR equations;
...
For a text-space program, the same mechanism records {py:func}`af.lm.complete <autoform.lm.complete>` as an equation instead of calling the provider during tracing.
