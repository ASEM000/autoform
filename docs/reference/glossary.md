...
| Transform | A function that consumes an IR and returns another IR. Current IR transforms include {py:func}`batch <autoform.batch>`, {py:func}`pushforward <autoform.pushforward>`, {py:func}`pullback <autoform.pullback>`, {py:func}`sched <autoform.sched>`, and {py:func}`dce <autoform.dce>`. |
